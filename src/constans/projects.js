export const projects = [
  {
    img: '/img/netflix-clone.png',
    technologies: ['React', 'Styled Components', 'Firebase', 'Jest'],
    url_website: 'https://netflix-clone-kohl-one.vercel.app/',
    url_github: 'https://github.com/AlexisToribio/netflix-clone',
  },
  {
    img: '/img/search-pokemon.png',
    technologies: ['Next (SSG - ISR - ISG)', 'NextUi'],
    url_website: 'https://pokemon-static-beta-virid.vercel.app/',
    url_github: 'https://github.com/AlexisToribio/pokemon-static',
  },
  // {
  //   img: '/img/openjira-clone.png',
  //   technologies: ['Next (SSR)', 'MaterialUi', 'MongoDB', 'Docker'],
  //   url_website: 'https://open-jira-six.vercel.app/',
  //   url_github: 'https://github.com/AlexisToribio/open-jira',
  // },
  {
    img: '/img/shopping-cart.png',
    technologies: ['React', 'CSS'],
    url_website: 'https://shopping-cart-dun-psi.vercel.app/',
    url_github: 'https://github.com/AlexisToribio/shopping-cart',
  },
  {
    img: '/img/tic-tac-toe.png',
    technologies: ['React', 'CSS'],
    url_website: 'https://tic-tac-toe-chi-indol.vercel.app/',
    url_github: 'https://github.com/AlexisToribio/tic-tac-toe',
  },
];
